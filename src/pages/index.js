...
import { Card, CardContent, Typography, Grid, Box, Alert, Backdrop, CircularProgress, Snackbar, Button } from '@mui/material'
import { IoMailSharp } from 'react-icons/io5'
import { PiUsersFourFill } from 'react-icons/pi'
import { FaUserTie, FaUserMd, FaUserNurse, FaUserGraduate, FaUserCog } from 'react-icons/fa'
import ImportantInfoDialog from 'src/components/ImportantInfoDialog '
...
const Dashboard = () => {
  ...
  const totalUsers = [
    {
      label: 'Total BO',
      value: totalUserAndTemplates.total_bo,
      icon: <FaUserTie size={40} style={{ color: '#4CAF50' }} />
    },
    {
      label: 'Total ABM',
      value: totalUserAndTemplates.total_abm,
      icon: <FaUserMd size={40} style={{ color: '#2196F3' }} />
    },
    {
      label: 'Total RSM',
      value: totalUserAndTemplates.total_rsm,
      icon: <FaUserNurse size={40} style={{ color: '#FFC107' }} />
    },
    {
      label: 'Total NSM',
      value: totalUserAndTemplates.total_nsm,
      icon: <FaUserGraduate size={40} style={{ color: '#E91E63' }} />
    },
    {
      label: 'Total GPM',
      value: totalUserAndTemplates.total_gpm,
      icon: <FaUserCog size={40} style={{ color: '#9C27B0' }} />
    }
  ]
  ...
  return (
    <>
     ...
     { (roleUser === "A") ? <ImportantInfoDialog open={dialogOpen} handleClose={() => setDialogOpen(false)} /> : null}
